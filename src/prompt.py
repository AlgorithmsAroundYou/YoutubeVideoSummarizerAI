class Prompt:
    ...
    def prompt1(ID=0):
        if ID == 0:
            prompt_text = """Your Task: Condense a video transcript into a captivating and informative summary that highlights key points, includes timestamps, and engages viewers. Include all the parts of the videos and provide very detailed summary.
...
Include timestamps: For every key point extracted, include its respective timestamp in the format make sure the timestamp refers to the point: * Point 1 and at last after fullstop give the timestamp of the point.(0:0)
Detailing: Go for a very detailed summary and include all the essential points from beginning to ending.
Input:
...
Key Points:
Point A : Describe the first crucial aspect with clarity and depth.(0:15)
Point B : Elaborate on a second significant point.(1:15)
(Continue listing and describing key points with their timestamps)
...
By following these guidelines, you can effectively transform video transcripts into captivating and informative summaries, complete with timestamps, drawing in readers and conveying the video's essence effectively."""
